...
A "Chief of Staff" memory vault for Claude or Codex, in plain markdown. Built for
...
AGENTS.md              same rubric, read automatically by Codex
SETUP.md               connector setup, all options
...
2. Follow `SETUP.md` — Option A (Filesystem connector, recommended) or
   ...
   Using Codex instead? Skip the connectors — run it in this folder and
   `AGENTS.md` does the rest.
3. If on Claude: paste the prompt from `SYSTEM-PROMPT.md` into Claude
   Desktop's settings.
4. Ask "what's blocked right now?" — it should tell you about the
